...
function c26202165.filter(c)
	return c:IsType(TYPE_MONSTER) and c:GetAttack()<=1500 and c:IsAbleToHand()
end

function c26202165.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c26202165.filter,tp,LOCATION_DECK,0,1,nil) end
